...
This file tells Codex how to operate the EU Commission agent system.
...
| `plugins/<domain>/.Codex-plugin/plugin.json` | Plugin manifest — skill registry, connectors |
| `plugins/<domain>/AGENTS.md` | Domain practice profile — playbook, house style, constraints |
...
4. Update the commissioner shortnames grid in `plugins/eu-simulation/AGENTS.md`.

**To add a new skill:**
1. Create `plugins/<domain>/skills/<skill-name>/SKILL.md` using the SKILL.md frontmatter convention.
2. Register it in `plugins/<domain>/.Codex-plugin/plugin.json` under `"skills"`.
3. Add a row to the Playbook table in `plugins/<domain>/AGENTS.md`.
